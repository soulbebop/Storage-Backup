...
> 사용자의 도메인에 백업할 서버를 등록하기 위해 agent 설치 명령의 파라미터로 도메인 정보를 입력합니다.
...
백업 서버와 통신을 위해 서버 Security Group에 아래 내용을 추가합니다.
...
도메인 정보가 `/TCBackup_alpha/faL6GUt2nboWNI0a`라면 다음과 같이 agent를 설치합니다.
...
* **Windows**
  ...
  Windows 용 agent는 [TOAST의 다운로드 페이지](http://docs.toast.com/ko/Download)에서 다운로드해 설치합니다. 설치 중 MC Server와 MC Domain 정보를 입력해야 합니다. MC Domain은 사용자의 도메인 정보입니다. MC Server는 다음 정보를 입력해 주십시오.
...
### 백업 agent 재등록
백업할 서버의 호스트 명이 변경되었다면 agent를 재등록해야 합니다. 재등록 명령은 다음과 같습니다.
...
### 백업 agent 종료
잠시 백업을 중단하고 싶다면 agent를 종료할 수 있습니다.
...
* **Windows**
    시스템 트레이에서 백업 agent 아이콘을 우클릭한 다음 종료 항목을 클릭합니다.

### 백업 agent 재시작
종료한 백업 agent를 다시 시작하려면 다음 명령을 사용합니다.
...
## 서버 등록
백업할 서버에 agent를 설치하면 서버 등록 화면의 서버 선택 항목에서 agent가 등록된 서버를 선택할 수 있습니다.
...
웹 콘솔에서 서버를 삭제한 후에는 반드시 서버에서 agent를 정지시키고 등록을 해지해야 합니다. 그렇지 않으면 해당 서버를 다시 등록할 수 없습니다. 해지 명령은 다음과 같습니다.
...
* **Windows**
    ...
    agent를 종료합니다. 다음에 다시 사용할 때는 클라이언트 활성화 항목을 열어 새로운 도메인 정보를 입력합니다.
